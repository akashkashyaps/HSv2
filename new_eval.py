...
Context Precision measures the proportion of relevant chunks within the retrieved contexts. It is computed as the mean of the precision@k values for each retrieved context chunk.

For a list of retrieved context chunks:
1. Let N be the total number of chunks in `retrieved_contexts`.
2. For each rank k (1 ≤ k ≤ N), define:
   - **precision@k = (Number of relevant chunks in the top k) / k.**
3. A chunk is considered **relevant** if it provides information that aligns with the **Reference** (and supports answering the **User Input**).
   - For each chunk at rank k, let the relevance indicator rₖ = 1 if the chunk is relevant, and 0 if it is not.
4. Then, **Context Precision** is given by:
   - Context Precision = (1/N) * ∑ₖ₌₁ᴺ precision@k

The final output should be a value between 0 and 1.
...
2. **Compute Precision@k:**  
   For each rank k from 1 to N, calculate:
   \[
   \text{precision@k} = \frac{\sum_{i=1}^{k} r_i}{k}
   \]

3. **Calculate Context Precision:**  
   Compute the mean of all the precision@k values:
   \[
   \text{Context Precision} = \frac{1}{N} \sum_{k=1}^{N} \text{precision@k}
   \]
...
3. **Metric Calculation:**  
   Let \( C \) be the total number of claims extracted from the reference. For each claim \( i \) (where \( i = 1, 2, ..., C \)), let \( a_i \) be its attribution indicator (1 if supported, 0 otherwise).  
   The **Context Recall** is calculated as:  
   \[
   \text{Context Recall} = \frac{\sum_{i=1}^{C} a_i}{C}
   \]
...
STRICTLY output only the value. NO additional information or formatting is required. You are FORBIDDEN from explaining your value. Stick to the recall calculation only.
"""
)
...
def test_single_evaluation(model_name, sample_data):
    """Test evaluation with one model"""
    print(f"\nTesting {model_name}...")
    ...
    for metric_name, prompt in metrics.items():
        try:
            ...
            result = chain.invoke({
                ...
                "retrieved_contexts": "\n".join([f"- {ctx}" for ctx in sample_data["retrieved_contexts"]])
            })
            
            clean_result = float(result.strip().replace('*', ''))
            results[metric_name] = clean_result
            print(f"{metric_name}: {clean_result} ({time.time()-start_time:.2f}s)")
            time.sleep(1)
...
def test_all_models():
    """Test evaluation with all models on sample data"""
    sample_data = {
        "user_input": "What causes seasons on Earth?",
        "reference": "Earth's seasons are caused by the tilt of its rotational axis relative to its orbital plane.",
        "response": "The seasons result from Earth's axial tilt of 23.5 degrees relative to its orbit around the Sun.",
        "retrieved_contexts": [
            "Earth's axial tilt is approximately 23.5 degrees",
            "Seasonal changes are primarily caused by axial tilt, not distance from the Sun"
        ]
    }
    
    print("Running comprehensive model tests...")
    metrics = context_precision | context_recall | context_entities_recall | noise_sensitivity | response_relevancy | faithfulness
    
    for model in EVALUATOR_MODELS:
        test_single_evaluation(model, sample_data, metrics)
        print("\n" + "="*50 + "\n")

def process_file(csv_path, metrics):
    """Process a single CSV file with all evaluator models"""
    ...
    for model in EVALUATOR_MODELS:
        ...
        # Process rows
        for idx, row in tqdm(df.iterrows(), total=len(df), desc=model_safe):
            try:
                contexts = "\n".join([f"- {ctx}" for ctx in row['retrieved_contexts']])
                
                for metric_name, prompt in metrics.items():
                    col_name = f"{metric_name}_{model_safe}"
                    if pd.isnull(row[col_name]):
                        ...
                        df.at[idx, col_name] = float(result.strip().replace('*', ''))
                        time.sleep(1)
    ...
    # Final save
    output_path = csv_path.replace('.csv', '_evaluated.csv')
...
if __name__ == "__main__":
    ...
    # 2. Confirm before full processing
